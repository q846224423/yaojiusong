...
public interface DoctorMapper {
    ...
    //查询单个医生提交的修改
    @Select("select * from doctorchange where dc_id = #{id}")
    public DoctorChange selectOneDc(int id);
    //增加医生修改数据
    @Insert("insert into doctorchange values(#{dc_name},#{dc_age},#{dc_sex},#{dc_tel},#{dc_kb_id},#{dc_money},#{dc_zc},#{dc_introduce},#{dc_head},#{dc_id})")
