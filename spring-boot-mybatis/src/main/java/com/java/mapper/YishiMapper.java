...
//查询所有问诊记录
	@Select("select * from wzjlu where user_id=${user_id} order by r_time desc")
	public List<Users_biger> wzjl(@Param("user_id") int user_id);
	...
	@Update("update record set isOpenDrug=1,yd_id=${yd} where isOpenDrug=0 and r_id=${yhid}")
	int updatestart(@Param("yhid") int yhid,@Param("yd") int yd);
	//通过医师ID查找处方信息s
