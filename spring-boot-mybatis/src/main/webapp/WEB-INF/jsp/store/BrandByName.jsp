...
<div class="i_bg">
    ...
    <div class="content mar_20">
        ...
        <div class="l_list">
			<div class="brand_t">所有商品</div>
            <div class="list_c">
<!-- 这里插入商品列表 -->
               
<div class="id">
<c:forEach items="${list }" var="menu3">
<!-- 每张图片的盒子，与图片个数相关 -->
<div class="div1" style="margin: 30px;">
<a href="ShowinfoServlet?id=${menu3.menu3id}"><img src="${menu3.ep_url}" /></a><br/>
${menu3.menu3name }<br/>
<strong>￥</strong><b>${menu3.ep_price }</b>&nbsp;&nbsp;&nbsp;&nbsp;剩余：${menu3.ep_stock }<br/>
<%-- 描述:${menu3.ep_description } --%>
</div>
</c:forEach>
</div>
 <div id="fy_tb">
<%
    int a =Integer.parseInt(String.valueOf(request.getSession().getAttribute("Bynamemax")));
    int b = Integer.parseInt(String.valueOf((request.getAttribute("page"))));
    String c = (String)(request.getAttribute("queryName"));
    %>
            
 <!-- 这里插入商品列表 -->
                <div class="pages">
                	<a href="QueryByName?page=<%=b-1%>&ybquery=<%=c %>" class="p_pre">上一页</a> 
             <%for(int i = 1;i<=a;i++){%>
    <a class="Cur" href="QueryByName?page=<%=i %>&ybquery=<%=c %>"><%=i %></a>
    <%}%><a href="QueryByName?page=<%=(b+1)%>&ybquery=<%=c %>"class="p_pre">下一页</a>
                </div>
