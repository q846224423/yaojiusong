...
<%-- 描述:${menu3.ep_description } --%>
 ...
 <div id="fy_tb">
<%
    int a =Integer.parseInt(String.valueOf(request.getSession().getAttribute("spidmax")));
    int b = Integer.parseInt(String.valueOf((request.getAttribute("page"))));
    int c = (Integer)(request.getAttribute("id"));
    %>
            
 <!-- 这里插入商品列表 -->
                <div class="pages">
                	<a href="QueryByidServlet?page=<%=b-1%>&id=<%=c %>" class="p_pre">上一页</a> 
             <%for(int i = 1;i<=a;i++){%>
    <a class="Cur" href="QueryByidServlet?page=<%=i %>&id=<%=c %>"><%=i %></a>
    <%}%><a href="QueryByidServlet?page=<%=(b+1)%>&id=<%=c %>"class="p_pre">下一页</a>
                </div>
